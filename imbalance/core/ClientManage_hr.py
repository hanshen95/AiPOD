...
from core.FedSkipClient_hr import FedSkipClient
...
class ClientManageHR(ClientManage):
    def __init__(self,args, net_glob, client_idx, dataset, dict_users, hyper_param, nets_loc) -> None:
        super().__init__(args, net_glob, client_idx, dataset, dict_users, hyper_param, nets_loc)
    ...
    def fed_skip(self, cs):
        ...
        for idx in self.client_idx:
            ...
            for name, w in net_loc.named_parameters():
       ...
       # skip communication or communicate and update correction term
        ...
        if f.data.item()==1:
            for idx in self.client_idx:
                ...
                c = cs[idx]
                i = 0
                for (name, z), (_, y) in zip(net_loc.named_parameters(),self.net_glob.named_parameters()):
                    if not "header" in name:
                        continue
