...
class SGDClient(Client):
    ...
    def train_epoch(self):
        ...
        optimizer = torch.optim.SGD([k for k in self.net.parameters() if k.requires_grad==True], lr=self.args.lr)
        c_optim = torch.optim.SGD([k for k in self.net.parameters() if k.requires_grad==True], lr=self.args.lr)
